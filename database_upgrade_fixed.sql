-- 修复版本：数据库升级脚本，使用正确的数据类型
-- 执行顺序：先备份数据，再执行升级

-- 1. 为 history 表添加 child_id 字段（使用 BIGINT 类型匹配 children.id）
ALTER TABLE history ADD COLUMN child_id BIGINT REFERENCES children(id);
...
-- 3. 为 tasks 和 rewards 表也添加 child_id（使用 BIGINT 类型）
ALTER TABLE tasks ADD COLUMN child_id BIGINT REFERENCES children(id);
ALTER TABLE rewards ADD COLUMN child_id BIGINT REFERENCES children(id);
